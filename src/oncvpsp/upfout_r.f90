...
subroutine upfout_r(lmax, lloc, rc, vkb, evkb, nproj, rr, vpuns, rho, rhomod, &
   ...
   !
   vpl(:, :) = 0.0d0
   call interpolate(rr, vpuns(1, lloc + 1), mmax, rl, vpl(1, lloc + 1), nrl)
   ...
   do l1 = 1, lmax + 1
      ...
      do ikap = 1, mkap
         do jj = 1, nproj(l1)
            ...
            call interpolate(rr, vkb(1, jj, l1, ikap), mmax, rl, vkbl(1, jj, l1, ikap), nrl)
   ...
   do ii = 1, nv
      ...
      nwfc = nwfc + mkap
      do ikap = 1, mkap
         call interpolate(rr, uua(1, ikap, ii), mmax, rl, uual(1, ikap, ii), nrl)
      end do
   end do
