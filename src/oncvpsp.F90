...
program oncvpsp
   ...
   character(len=1024) :: upffile
   character(len=16) :: upfgrid
   character(len=1024) :: psp8file
   character(len=1024) :: psmlfile
      ...
      do i = 1, command_argument_count()
         ...
         select case(arg)
          case('-h', '--help')
            ...
            write (stdout, '(t2,a)') '-h, --help       Show this help message and exit'
            write (stdout, '(t2,a)') '-v, --version    Show version information and exit'
            write (stdout, '(t2,a)') '-i, --input      Specify input (dat) file'
#if (defined WITH_TOML)
            write (stdout, '(t2,a)') '-t, --toml-input Specify input (toml) file'
#endif
            stop 0
          case('-v', '--version')
   ...
   ios = 0
   if (input_mode == INPUT_STDIN .or. input_mode == INPUT_TEXT) then
      ...
      if (ios /= 0) then
         write(6,'(a)') 'oncvpsp: ERROR opening input file'
         stop 1
      end if
      ...
      if (input_mode == INPUT_TEXT) then
         close(unit)
      end if
   end if

#if (defined WITH_TOML)
   if (input_mode == INPUT_TOML) then
      open(newunit=unit, file=input_filename, status='old', action='read', iostat=ios)
      if (ios /= 0) then
         write(6,'(a)') 'oncvpsp: ERROR opening input file'
         stop 1
   ...
   call psfileout(lmax, lloc, rc, vkb, evkb, nproj, rr, vpuns, rho, rhomod, irct, srel, &
