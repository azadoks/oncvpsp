...
&                  epsh1, epsh2, depsh, rlmax, psfile)
   ...
   use libxc_list_m  ! For ease of libxc handling
   implicit none
   ...
   integer :: irct
   !> whether it is scalar-relativistic or not
   logical :: srel
   !> maximum number of projectors for any l
   integer :: mxprj
...
&                  epsh1, epsh2, depsh, rlmax, psfile)
   ...
   !> vpuns  unscreened semi-local pseudopotentials (vp(:,5) is local potential
   !>   if linear combination is used)
   real(dp), target :: vpuns(mmax, 5, 2)
   !> rho  valence pseudocharge
   real(dp), target :: rho(mmax)
   !> vkb  VKB projectors
   real(dp), target :: vkb(mmax, mxprj, 4, 2)
   !> rhomod  model core charge
   real(dp), target :: rhomod(mmax, 5)
   !> rc  core radii
   real(dp) :: rc(6)
   ...
   real(dp), target :: vsr(mmax, 4, 4)
   real(dp), target :: vso(mmax, 4, 4)
   real(dp) :: esr(4, 4)
   real(dp) :: eso(4, 4)
   !> atsym  atomic symbol
   character*2 :: atsym
